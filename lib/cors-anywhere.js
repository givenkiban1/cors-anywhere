...
//This is a pull request made by Fabian Sellmann, from his fork of the original repo, 
//Pull request title: allow cookies behind flag (bare minimum)
//pull request #56 #148
//this pull request was however closed by Rob.

//I, Given, have however made changes to this code to make it work as i desire it to.
...
function withCORS(headers, request) {
  ...
  //the code below sets which headers must be returned in the response
  //i've added given, given2 as who ever sends out a request, is expecting their cookies as either value.
  headers['access-control-expose-headers'] = Object.keys(headers).join(',') + ",Given,Given2";
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  // Strip or update cookies
  
  //the line below must be changed.
  //once i figure out how to send enableCookies as true to this server

  if(true){//req.corsAnywhereRequestState.enableCookies
    var pathRegex = /path="((\/|\w|\?|\=|\:|\%|\.|\&|\#)*?)"/gm
    function updateCookies(cookies){
      //change to be made here

      //check what is the type of cookie -- string or object
      //if it's an object, leave it as it is
      //if it's a string, perform the instructions below
    ...
    if(typeof proxyRes.headers['set-cookie'] === "object"){
      //the line below has been commented out because, the site i will be trying to access
      //returns a json/object type cookie
      //the line below parses the cookie as a string

      //perhaps what can be done in future is to change the updateCookies function as described @the function

      //proxyRes.headers['set-cookie'] = updateCookies(proxyRes.headers['set-cookie'])
      proxyRes.headers['Given'] = proxyRes.headers['set-cookie'];

    }
    if(typeof proxyRes.headers['set-cookie2'] === "object"){
      //proxyRes.headers['set-cookie2'] = updateCookies(proxyRes.headers['set-cookie2'])
      proxyRes.headers['Given2'] = proxyRes.headers['set-cookie2'];

    }

    //in the code above, i have rename assigned new headers, Given and Given2, to the relevant set cookie values
    //because the when the cookies are returned as set-cookie, they get intercepted bby the browsers,
    //preventing me from accessing them. so what i've done is simply returning the cookies as a different header
    //i also had to add these 2 new headers to the access-control-expose-headers header in the withCors function.
    else{
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    //gift is a cookie that is sent via a request
    //i take this cookie and assign it to the new cookie header which i've created above.

    if (typeof req.headers["gift"] === "string")
        req.headers["cookie"] = req.headers["gift"]
